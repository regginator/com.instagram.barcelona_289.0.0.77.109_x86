package p000X;
/* renamed from: X.5KB  reason: invalid class name */
/* loaded from: classes3.dex */
public final class C5KB extends C0SZ implements InterfaceC21811Blt {
    public final String A00;
    public final String A01;

    @Override // p000X.InterfaceC21811Blt
    public final C5KB Czo() {
        return this;
    }

    public final boolean equals(Object obj) {
        if (this != obj) {
            if (obj instanceof C5KB) {
                C5KB c5kb = (C5KB) obj;
                if (!C0OR.A0I(this.A00, c5kb.A00) || !C0OR.A0I(this.A01, c5kb.A01)) {
                }
            }
            return false;
        }
        return true;
    }

    public final int hashCode() {
        return C25960wt.A06(this.A01, C25930wq.A03(this.A00));
    }

    public C5KB(String str, String str2) {
        C25920wp.A1R(str, str2);
        this.A00 = str;
        this.A01 = str2;
    }

    @Override // p000X.InterfaceC21811Blt
    public final String BKR() {
        return this.A01;
    }

    @Override // p000X.InterfaceC21811Blt
    public final String getId() {
        return this.A00;
    }
}
